...
const RegisterLinkedInActionsEnhanced: React.FC = () => {
  ...
  const applyPersonaConstraints = (content: string, constraints: any) => {
    ...
    if (constraints.sentence_metrics?.average_sentence_length_words) {
      const targetLength =
        constraints.sentence_metrics.average_sentence_length_words;
      // This is a simplified example - in practice, you'd use more sophisticated NLP
      console.log(
        `🎭 Applying persona sentence length constraint: ${targetLength} words average`,
      );
    }
    ...
    if (constraints.lexical_features?.go_to_words?.length > 0) {
      console.log(
        `🎭 Using persona go-to words: ${constraints.lexical_features.go_to_words.join(", ")}`,
      );
    }

    if (constraints.lexical_features?.avoid_words?.length > 0) {
      console.log(
        `🎭 Avoiding persona avoid words: ${constraints.lexical_features.avoid_words.join(", ")}`,
      );
    }
  ...
  useCopilotActionTyped({
    ...
    description:
      "Generate a professional LinkedIn post optimized for your writing persona and platform constraints",
    parameters: [
    ...
    handler: async (args: any) => {
      ...
      const personaInfo = corePersona
        ? `using ${corePersona.persona_name} persona`
          ...
          detail: {
            steps: [
              ...
              { id: "grounding", label: "Applying AI grounding" },
              {
                ...
                label: "Generating persona-optimized content",
              },
              {
                id: "persona_validation",
                label: "Validating against persona constraints",
              },
      ...
      if (args?.refine_existing) {
        ...
        if (currentDraft) {
          console.log(
            `🎭 Refining existing content: ${currentDraft.substring(0, 100)}...`,
          );
        }
      }
      ...
      const maxLength =
        ...
        DEFAULT_LINKEDIN_POST_MAX_LENGTH;
      const optimalLength =
        platformPersona?.content_format_rules?.optimal_length ||
        "150-300 words";

      console.log(
        `🎭 Persona constraints applied: Max ${maxLength} chars, Optimal: ${optimalLength}`,
      );
      ...
      if (res.success && res.data) {
        ...
        if (corePersona && platformPersona) {
          enhancedContent = applyPersonaConstraints(enhancedContent, {
            sentence_metrics:
              corePersona.linguistic_fingerprint?.sentence_metrics,
            lexical_features:
  ...
  useCopilotActionTyped({
    ...
    description:
      "Generate a LinkedIn article optimized for your writing persona and platform constraints",
    parameters: [
      { name: "topic", type: "string", required: false },
      { name: "industry", type: "string", required: false },
      {
        ...
        type: "string",
        required: false,
        description: "Short, Medium, or Long article",
      },
    ],
    handler: async (args: any) => {
      // Persona-aware progress tracking
        ...
        new CustomEvent("linkedinwriter:progressInit", {
          detail: {
            steps: [
              ...
              {
                id: "content_generation",
                label: "Generating persona-optimized article",
              },
              {
                id: "persona_validation",
                label: "Validating against persona constraints",
      ...
      console.log(
        `🎭 Generating ${articleLength} article with persona constraints`,
      );
      ...
      window.dispatchEvent(
        new CustomEvent("linkedinwriter:progressStep", {
          detail: {
            id: "finalize",
            status: "completed",
            message: "Article generation placeholder",
          },
        }),
      );
  ...
  useCopilotActionTyped({
    ...
    description:
      "Validate existing content against your writing persona and suggest improvements",
    parameters: [
      {
        name: "content",
        type: "string",
    ...
    handler: async (args: any) => {
      ...
      const validation = {
        sentence_length: {
          current: content.split(".").filter((s: string) => s.trim().length > 0)
            .length,
          target:
            corePersona.linguistic_fingerprint?.sentence_metrics
      ...
      const goToWords =
        corePersona.linguistic_fingerprint?.lexical_features?.go_to_words || [];
      const avoidWords =
        corePersona.linguistic_fingerprint?.lexical_features?.avoid_words || [];
      ...
      avoidWords.forEach((word) => {
        ...
        if (matches) {
          ...
          validation.vocabulary_usage.suggestions.push(
            `Consider replacing "${word}" with a more aligned word`,
          );
        }
      });
      ...
      const charLimit =
        platformPersona.content_format_rules?.character_limit || 3000;
      if (content.length > charLimit) {
        validation.platform_compliance.status = "exceeds_limit";
        validation.platform_compliance.suggestions = [
          `Content exceeds ${charLimit} character limit by ${content.length - charLimit} characters`,
        ];
      } else {
      ...
      return {
        ...
        recommendations: validation.vocabulary_usage.suggestions.concat(
          validation.platform_compliance.suggestions || [],
        ),
      };
    },
  ...
  useCopilotActionTyped({
    ...
    description:
      "Get personalized writing suggestions based on your persona and LinkedIn platform",
    parameters: [
      {
        name: "content_type",
        type: "string",
    ...
    handler: async (args: any) => {
      ...
      const suggestions = {
        writing_style: {
          sentence_structure:
            corePersona.linguistic_fingerprint?.sentence_metrics
              ?.preferred_sentence_type || "balanced",
          tone_recommendation:
            platformPersona?.tonal_range?.default_tone ||
        ...
        platform_optimization: {
          ...
          optimal_length:
            ...
            "150-300 words",
          hashtag_strategy:
            platformPersona.lexical_features?.hashtag_strategy ||
        ...
        persona_specific: {
          ...
          avoid_words:
            ...
            [],
          rhetorical_style:
            corePersona.linguistic_fingerprint?.rhetorical_devices?.metaphors ||
            "business-focused",
        },
      };
