...
export const VALID_SEARCH_ENGINES = ["google", "tavily", "exa"] as const;
...
export function logAssistant(content: string) {
  try {
    window.dispatchEvent(
      new CustomEvent("linkedinwriter:assistantMessage", {
        detail: { content },
      }),
    );
  } catch {}
...
export function mapPostType(postType: string | undefined): LinkedInPostType {
  ...
  if (pt.includes("thought") || pt.includes("leadership"))
    return LinkedInPostType.THOUGHT_LEADERSHIP;
  if (pt.includes("news") || pt.includes("industry"))
    return LinkedInPostType.INDUSTRY_NEWS;
  if (pt.includes("personal") || pt.includes("story"))
    return LinkedInPostType.PERSONAL_STORY;
  if (pt.includes("company") || pt.includes("update"))
    return LinkedInPostType.COMPANY_UPDATE;
  if (pt.includes("poll") || pt.includes("question"))
...
export function mapTone(
  ...
  if (t.includes("authoritative") || t.includes("expert"))
    return LinkedInTone.AUTHORITATIVE;
  if (t.includes("conversational") || t.includes("casual"))
    return LinkedInTone.CONVERSATIONAL;
  if (t.includes("inspirational") || t.includes("motivational"))
    return LinkedInTone.INSPIRATIONAL;
  if (t.includes("educational") || t.includes("informative"))
    return LinkedInTone.EDUCATIONAL;
  if (t.includes("friendly") || t.includes("approachable"))
...
export function getIndustryHashtags(industry: string): string[] {
  ...
  return (
    PROFESSIONAL_HASHTAGS[industryKey as keyof typeof PROFESSIONAL_HASHTAGS] ||
    PROFESSIONAL_HASHTAGS.general
  );
}

export function getProfessionalSuggestions(
  contentType: string,
  industry: string,
): string[] {
  const suggestions = {
...
export function getPersonalizedPlaceholder(
  ...
  const contentPlaceholders =
    placeholders[contentType as keyof typeof placeholders];
  if (contentPlaceholders) {
    return (
      contentPlaceholders[fieldType as keyof typeof contentPlaceholders] ||
...
export function validateLinkedInContent(content: string): {
  ...
  if (content.length > HARD_LIMIT) {
    issues.push(
      `Content exceeds LinkedIn post limit (${HARD_LIMIT} characters)`,
    );
  }
