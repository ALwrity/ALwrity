...
export function measureProfilePictureCenterX(): number | null {
  ...
  const hubEl = document.querySelector('.linkedin-dashboard-hero-hub') as HTMLElement | null;
  if (hubEl) {
...
export function syncHubHeaderPersona(
  ...
  if (!desktopViewport || !canvas || !document.body.classList.contains('linkedin-dashboard-view')) {
    clearHubHeaderPersonaSync();
    return;
  }

  const headerRow = document.querySelector('.linkedin-writer-header-row') as HTMLElement | null;
  if (!headerRow) {
    clearHubHeaderPersonaSync();
    return;
  }
  ...
  const previous = document.body.style.getPropertyValue(HUB_HEADER_PERSONA_LEFT_CSS_VAR);
