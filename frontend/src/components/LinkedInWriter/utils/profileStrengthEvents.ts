...
export const PROFILE_STRENGTH_UPDATED_EVENT =
  "linkedinwriter:profileStrengthUpdated";
...
export function dispatchProfileStrengthUpdated(
  ...
  window.dispatchEvent(
    new CustomEvent<ProfileStrengthUpdatedDetail>(
...
export function dispatchLinkedInPersonaUpdated(
  snapshot: LinkedInPersonaSnapshot,
): void {
  window.dispatchEvent(
...
export const LINKEDIN_PRIORITY_ACTION_EVENT =
  "linkedinwriter:priorityActionUpdated";
...
export function selectTopPriorityAction(
  ...
  const toOptSnap = (
    item: LinkedInProfileOptimizationItem,
  ): PriorityActionSnapshot => ({
    type: "optimization",
    title: item.issue,
  ...
  // 1. Quick win
  const quickWin = optItems.find(
    (i) => i.impact === "High" && i.effort === "Low",
  );
...
export function dispatchLinkedInPriorityAction(
  snapshot: PriorityActionSnapshot | null,
): void {
  // Persist so UserBadge can restore the card across menu open/close cycles in the same session.
  try {
    if (snapshot)
      sessionStorage.setItem(PRIORITY_SESSION_KEY, JSON.stringify(snapshot));
  } catch (_) {
    /* sessionStorage unavailable — no-op */
  }

  window.dispatchEvent(
    new CustomEvent<PriorityActionSnapshot | null>(
