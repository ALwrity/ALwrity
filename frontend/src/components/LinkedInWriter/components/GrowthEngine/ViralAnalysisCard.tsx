...
export const ViralAnalysisCard: React.FC<ViralAnalysisCardProps> = React.memo(
  ({ industry, patterns, topRecommendation, dataSourceSummary, onDismiss }) => {
    if (!patterns || patterns.length === 0) {
      return (
        <EmptyState
          icon="🔥"
          message={`No viral patterns available for ${industry} right now.`}
        />
      );
    }
        ...
        <div style={headerRow}>
          ...
          {onDismiss && (
            <button
              ...
              style={dismissBtn}
              title="Dismiss"
              aria-label="Dismiss viral analysis"
            >
              ✕
            </button>
          )}
