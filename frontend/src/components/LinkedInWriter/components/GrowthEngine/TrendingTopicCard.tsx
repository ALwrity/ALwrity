...
export const TrendingTopicCard: React.FC<TrendingTopicCardProps> = React.memo(
  ({ industry, topics, dataSourceSummary, onPostAbout, onDismiss }) => {
    if (!topics || topics.length === 0) {
      return (
        <EmptyState
          icon="📈"
          message="No trending topics available right now. Check back later."
        />
      );
    }
        ...
        <div style={headerRow}>
          ...
          {onDismiss && (
            <button
              ...
              style={dismissBtn}
              title="Dismiss"
              aria-label="Dismiss trending topics"
            >
              ✕
            </button>
          )}
          ...
          {topics.map((t, i) => (
            ...
            >
              ...
              <span
                style={{
                  background: (
                    CONFIDENCE_COLORS[t.confidence] || CONFIDENCE_COLORS.medium
                  ).bg,
                  color: (
                    CONFIDENCE_COLORS[t.confidence] || CONFIDENCE_COLORS.medium
                  ).text,
