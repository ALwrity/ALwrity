...
export const ContentGapCard: React.FC<ContentGapCardProps> = React.memo(
  ({ gaps, dataSourceSummary, onDismiss, onGeneratePost }) => {
    ...
    if (gaps.length === 0) {
      return (
        <EmptyState
          icon="🔍"
          message="No content gaps identified. Your content strategy looks well-rounded!"
        />
      );
    }
        ...
        <div style={headerRow}>
          ...
          {onDismiss && (
            <button
              ...
              style={dismissBtn}
              title="Dismiss"
              aria-label="Dismiss content gap analysis"
            >
              ✕
            </button>
          )}
...
const GapRow: React.FC<GapRowProps> = React.memo(
  ({ gap, index, onDismiss, onGeneratePost }) => {
