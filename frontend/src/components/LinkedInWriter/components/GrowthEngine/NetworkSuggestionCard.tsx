...
export const NetworkSuggestionCard: React.FC<NetworkSuggestionCardProps> =
  React.memo(({ suggestions, dataSourceSummary }) => {
    ...
    if (suggestions.length === 0) {
      return (
        <EmptyState
          icon="🤝"
          message="No network suggestions available. Connect your LinkedIn account for personalized recommendations."
        />
      );
    }
...
const PersonRow: React.FC<PersonRowProps> = React.memo(
  ({ person, index, copied, onCopy, onDismiss }) => {
    return (
      <div style={rowBase}>
        <div
          style={{
        ...
        >
          ...
          <span
            style={{
              background: (
                CONFIDENCE_COLORS[person.confidence] || CONFIDENCE_COLORS.medium
              ).bg,
              color: (
                CONFIDENCE_COLORS[person.confidence] || CONFIDENCE_COLORS.medium
              ).text,
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <button
            ...
            aria-label={
              copied ? "Connection note copied" : "Copy connection note"
            }
          >
