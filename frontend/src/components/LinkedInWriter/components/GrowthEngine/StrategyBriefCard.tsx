  ...
  ({
    ...
    if (!dailyPosts || dailyPosts.length === 0) {
      return (
        <EmptyState
          icon="📅"
          message="Weekly strategy not available. Check back after connecting your LinkedIn account."
        />
      );
    }
        ...
        <div style={headerRow}>
          ...
          {onDismiss && (
            <button
              ...
              style={dismissBtn}
              title="Dismiss"
              aria-label="Dismiss weekly strategy"
            >
              ✕
            </button>
          )}
...
const DayRow: React.FC<DayRowProps> = React.memo(
  ({ post, theme, onGeneratePost }) => {
      ...
      <div style={rowBase}>
        ...
        >
          ...
          <span
            style={{
              background: (
                CONFIDENCE_COLORS[post.confidence] || CONFIDENCE_COLORS.medium
              ).bg,
              color: (
                CONFIDENCE_COLORS[post.confidence] || CONFIDENCE_COLORS.medium
              ).text,
        ...
        <div
          style={{ fontSize: 11, color: colors.textMedium, lineHeight: 1.4 }}
        >
          💡 {post.why_this_works}
        </div>
        ...
        {onGeneratePost && (
          <button
            onClick={() =>
              onGeneratePost({
                topic: post.headline,
                context: `Weekly strategy: ${post.day} - ${post.content_type} post. Theme: "${theme}". Hook: ${post.hook}`,
              })
            }
