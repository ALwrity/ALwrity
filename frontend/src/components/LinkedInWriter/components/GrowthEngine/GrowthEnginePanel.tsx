...
export const GrowthEnginePanel: React.FC<GrowthEnginePanelProps> = ({
  ...
  const [consolidated, setConsolidated] =
    useState<ConsolidatedGrowthResponse | null>(null);
        ...
        setConsolidated((prev) => {
          ...
          switch (key) {
            case "trending":
              ...
              break;
            case "network":
              updated.network_suggestions =
                result as NetworkSuggestionsResponse;
              break;
            case "engagement":
              updated.engagement_opportunities =
                result as EngagementOpportunitiesResponse;
              break;
            case "viral":
  ...
  const handlePostAbout = useCallback(
    (topic: string, hook: string) => {
      openPostModal(topic, `Topic: ${topic}\nSuggested hook: ${hook}`);
    },
    [openPostModal],
  );

  const handleOpenModalFromCard = useCallback(
  ...
  if (panelState === "idle") {
    return (
      <div style={pageShell()}>
        ...
        <p
          style={{
            margin: embedded ? "0 0 16px" : "0 0 24px",
            fontSize: 13,
            color: colors.textSecondary,
          }}
        >
          AI-powered insights to grow your LinkedIn reach. Data-backed,
  ...
  if (c.network_suggestions?.generated_at)
    allGeneratedAts.push(c.network_suggestions.generated_at);
  if (c.engagement_opportunities?.generated_at)
    allGeneratedAts.push(c.engagement_opportunities.generated_at);
  if (c.viral_analysis?.generated_at)
    allGeneratedAts.push(c.viral_analysis.generated_at);
  if (c.weekly_strategy?.generated_at)
    allGeneratedAts.push(c.weekly_strategy.generated_at);
  if (c.content_gaps?.generated_at)
    allGeneratedAts.push(c.content_gaps.generated_at);
  if (c.brand_scorecard?.generated_at)
    allGeneratedAts.push(c.brand_scorecard.generated_at);

  const oldestMs =
    allGeneratedAts.length > 0
  ...
  const hasContent =
    (trending?.trending_topics?.length ?? 0) > 0 ||
          ...
          >
            {!embedded && (
              <div>
                ...
                >
                  AI-powered insights. Refresh individual cards to get fresh
            ...
            {cardWrapper(
              ...
              <div>
                ...
                >
                  <span style={{ fontSize: 11, color: colors.textSecondary }}>
                    Updated{" "}
                    {formatTimeAgo(engagement_opportunities.generated_at)}
                  </span>
                ...
                <EngagementCard
                  ...
                  dataSourceSummary={
                    engagement_opportunities.data_source_summary
                  }
            ...
            <div style={{ padding: 16, maxHeight: "60vh", overflow: "auto" }}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setCreationModal((p) => ({ ...p, context: e.target.value }))
                  }
