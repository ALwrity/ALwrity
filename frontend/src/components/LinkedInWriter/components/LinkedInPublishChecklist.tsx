...
const BP_TOOLTIPS: Record<string, string> = {
  ...
  hook: "The first line determines if readers click “see more”. Use a surprising stat, a question, or a counterintuitive claim.",
...
export const LinkedInPublishChecklist: React.FC<
  LinkedInPublishChecklistProps
> = ({ draft, hasMedia, compact = false, softOnly = false }) => {
  ...
  return (
    <Box
      sx={{ display: "flex", flexDirection: "column", gap: compact ? 0.75 : 1 }}
    >
      {!compact && (
        <Typography
          variant="caption"
      ...
      {items.map((item) => {
        ...
        const row = (
          ...
          >
            <Typography
              component="span"
              sx={{ fontSize: 14, lineHeight: "20px", flexShrink: 0 }}
            >
              {colors.icon}
            </Typography>
            <Box sx={{ minWidth: 0 }}>
              <Typography
                sx={{ fontWeight: 600, fontSize: 12, color: "#374151" }}
              >
                {item.label}
                {item.severity === "hard" ? (
                  <Typography
