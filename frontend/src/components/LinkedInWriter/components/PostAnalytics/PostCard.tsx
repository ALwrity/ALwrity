...
export const PostCard: React.FC<PostCardProps> = React.memo(
  ({ post, onGenerateSimilar }) => {
    ...
    const displayText =
      expanded || !isTruncated ? post.text || "(No text)" : previewText;
    const engagementRatePct = (post.engagement.engagement_rate * 100).toFixed(
      1,
    );
    ...
    return (
      <article style={cardBase} aria-label="LinkedIn post">
        ...
        >
          <MetricPill
            label="Reactions"
            value={formatMetric(post.engagement.reactions)}
          />
          <MetricPill
            label="Comments"
            value={formatMetric(post.engagement.comments)}
          />
          <MetricPill
            label="Reposts"
            value={formatMetric(post.engagement.reposts)}
          />
          <MetricPill
            label="Impressions"
            value={formatMetric(post.engagement.impressions)}
          />
          <MetricPill
            label="Eng. rate"
            value={`${engagementRatePct}%`}
            highlight
          />
          {post.engagement.clicks > 0 && (
            <MetricPill
              label="Clicks"
              value={formatMetric(post.engagement.clicks)}
              accent
            />
          )}
