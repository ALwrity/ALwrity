...
export const PostTimelineChart: React.FC<PostTimelineChartProps> = ({
  posts,
  onSelectPost,
}) => {
  ...
  const values = useMemo(
    () => sorted.map((p) => getMetricValue(p, metric)),
    [sorted, metric],
  );
  ...
  const points = sorted.map((post, i) => {
    const x =
      SVG_PADDING.left +
      (sorted.length > 1 ? (i / (sorted.length - 1)) * innerW : innerW / 2);
    const y =
      SVG_PADDING.top + innerH - ((values[i] - minVal) / range) * innerH;
...
function LegendDot({
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
      {dashed ? (
        <div
          style={{ width: 14, height: 1, borderTop: `2px dashed ${color}` }}
        />
      ) : (
        <div
          style={{
