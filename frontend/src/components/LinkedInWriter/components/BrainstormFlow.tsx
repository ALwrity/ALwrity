...
import MySavedIdeas, {
  type SavedBrainstormIdea,
} from "./Brainstorm/MySavedIdeas";
...
const BrainstormFlow: React.FC<BrainstormFlowProps> = ({
  ...
  const [personalizedStage, setPersonalizedStage] = useState<
    "idle" | "loading" | "results"
  >("idle");
  const [personalizedIdeas, setPersonalizedIdeas] = useState<
  ...
  const lastPersonalizeOptionsRef = useRef<{
    seed: string;
    ...
    try {
      ...
      if (Array.isArray(res.data?.ideas)) {
        setSavedPromptHashes(
          new Set(
            res.data.ideas.map((it: SavedBrainstormIdea) =>
  ...
  const handleSaveIdea = useCallback(
    async (idx: number) => {
      ...
      try {
        ...
        setSaveError(
          e?.response?.data?.detail || e?.message || "Failed to save idea",
        );
        setSavingIndex(null);
      }
    },
  ...
  const getCachedIdeas = useCallback(
    (cacheKey: string): BrainstormCacheData | null => {
      try {
        const cached = sessionStorage.getItem(cacheKey);
        if (cached) {
          const data = JSON.parse(cached);
          if (
            isBrainstormCacheData(data) &&
  ...
  useEffect(() => {
    const handler = async (ev: any) => {
      ...
      try {
        ...
        console.log(
          "[BrainstormFlow] finalSeed:",
          JSON.stringify(finalSeed),
          "hasOptions:",
          !!options &&
            (options.usePersona ||
              options.includeTrending ||
              options.remarketContent),
        );
        ...
        if (hasOptions) {
          ...
          lastPersonalizeOptionsRef.current = {
            seed: finalSeed || "",
            options: {
              usePersona: options.usePersona || false,
              includeTrending: options.includeTrending || false,
              remarketContent: options.remarketContent || false,
            },
          };
          try {
            const res = await aiApiClient.post(
              "/api/brainstorm/personalized-ideas",
              {
                seed: finalSeed || "",
                count: 5,
        ...
        const interval = setInterval(() => {
          setLoaderMessageIndex((idx) =>
            Math.min(idx + 1, loaderMessages.length - 1),
          );
        }, 700);
        ...
        try {
          ...
          const srcList = Array.isArray(ir.data?.sources)
            ? ir.data.sources
            : [];
          setIdeas(list);
          setSources(srcList);
        ...
        window.dispatchEvent(
          new CustomEvent("linkedinwriter:cancelBrainstorm"),
        );
        setBrainstormVisible(false);
        setStage("idle");
      } finally {
    ...
    return () =>
      window.removeEventListener("linkedinwriter:runBrainstormIdeas", handler);
  }, [
    corePersona,
    platformPersona,
    loaderMessages,
    ...
    <>
            ...
            <div style={{ flex: 1, overflow: "auto" }}>
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            ...
                            window.dispatchEvent(
                              new CustomEvent(
                                "linkedinwriter:runBrainstormIdeas",
                                {
                                  detail: {
                    ...
                    <>
                      ...
                      >
                        {ideas.length > 0
                          ...
                          : "No ideas found. Try a different topic."}
                        {isUsingCache && (
                          <span
                            style={{
                              fontSize: 12,
              ...
              {personalizedStage === "loading" &&
                (() => {
                  ...
                  if (hasPersona)
                    items.push(
                      "Reading your LinkedIn profile & communication style",
                    );
                  if (hasTrending)
                    items.push("Scanning industry trends & growth insights");
                  if (hasRemarket)
                    items.push(
                      "Reviewing your generated content & saved ideas",
                    );
                            ...
                            new CustomEvent(
                              "linkedinwriter:runBrainstormIdeas",
                              {
                                detail: {
                                  ...
                                  options:
                                    lastPersonalizeOptionsRef.current.options,
                                  forceRefresh: true,
                                },
                              },
                            ),
      ...
      <MySavedIdeas
        ...
        onUseInCopilot={(prompt: string) => {
          window.dispatchEvent(
            new CustomEvent("linkedinwriter:copilotSeedFromPrompt", {
              detail: { prompt },
            }),
          );
