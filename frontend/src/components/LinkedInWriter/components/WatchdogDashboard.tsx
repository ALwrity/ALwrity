...
import {
  linkedInWatchdogApi,
  type WatchdogUpdate,
  type WatchdogIndustry,
  type WatchdogCompany,
  type WatchdogPerson,
  type MonitorStatus,
...
export const WatchdogDashboard: React.FC<WatchdogDashboardProps> = ({
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (!isFirstAutoRefresh.current && newUnread > prevUnreadRef.current) {
          ...
          setToastMessage(
            `${delta} new update${delta > 1 ? "s" : ""} available`,
          );
        }
  ...
  const getMonitorCount = (
    ...
    const cat = monitors.filter(
      (m) => m.category === category && m.reference_id === refId,
    );
    return {
      active: cat.filter((m) => m.status === "active").length,
      total: cat.length,
    };
  };

  const hasMonitors = monitors.some((m) => m.status === "active");
  const totalActiveMonitors = monitors.filter(
    (m) => m.status === "active",
  ).length;
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      if (category === "industry") await linkedInWatchdogApi.deleteIndustry(id);
        ...
        await linkedInWatchdogApi.deleteCompany(id);
      else if (category === "person")
        await linkedInWatchdogApi.deletePerson(id);
  ...
  const handleGenerateInModal = useCallback(async () => {
    ...
    generatePost,
    creationModal.topic,
    creationModal.context,
    userPreferences,
  ]);

  const pollLabel =
    POLL_INTERVAL_OPTIONS.find((o) => o.ms === pollIntervalMs)?.label ||
    "Manual";
  ...
  const handlePersonalize = async () => {
    ...
    try {
      ...
      const industry =
        (profileRes.profile_context as any)?.industry ||
      ...
      setError(
        err?.response?.data?.detail ||
          err?.message ||
          "Could not load profile. Connect LinkedIn first.",
      );
    } finally {
                  ...
                  {POLL_INTERVAL_OPTIONS.map((opt) => (
                    <button
                      ...
                      style={{
                        ...
                        padding: "8px 14px",
                        background:
                          pollIntervalMs === opt.ms ? "#eff6ff" : "#fff",
                        color:
                          pollIntervalMs === opt.ms ? "#0a66c2" : "#374151",
                        border: "none",
                        cursor: "pointer",
                        fontSize: 12,
                        fontWeight: pollIntervalMs === opt.ms ? 700 : 500,
                        textAlign: "left",
                      }}
                      onMouseEnter={(e) => {
            ...
            (watchedItems.length > 0 || filteredUpdates.length > 0) && (
              <div
                style={{
                  ...
                  height: 2,
                  background:
                    "linear-gradient(90deg, #0a66c2 0%, #60a5fa 50%, #0a66c2 100%)",
          ...
          filteredUpdates.length === 0 ? (
            ...
            <div style={{ padding: "32px 20px" }}>
              ...
              <div style={{ textAlign: "center", marginBottom: 28 }}>
                ...
                >
                  Never miss a trend. We monitor the web for news about
                  industries, companies, and people you care about — so you can
                  turn breaking stories into LinkedIn posts in one click.
                </div>
              </div>
              ...
              >
                ...
                >
                  {personalizing
                    ? "Loading..."
                    : "🎯 Auto-add from LinkedIn Profile"}
                </button>
              </div>
              ...
              >
                💡 <strong>Pro tip:</strong> Click "Auto-add from LinkedIn
                Profile" to instantly start monitoring your industry and company
                — no typing required.
              </div>
            </div>
          ) : (
            <>
              ...
              {watchedItems.length > 0 && (
                <div style={{ marginBottom: 14 }}>
                  <div
                    className="watchdog-monitored-label"
                    style={{
                      fontSize: 11,
                      fontWeight: 600,
                      color: "#9ca3af",
                  ...
                  >
                    Monitored {tabLabels[tab].toLowerCase()} (
                    {watchedItems.length})
                  </div>
                  <div
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            style={{
                              fontWeight: 500,
                              color: isDeleting ? "#dc2626" : "#374151",
                              flex: 1,
                            }}
                          >
                            {(item as any).name || ""}
              ...
              {(watchedItems.length > 0 || filteredUpdates.length > 0) && (
                ...
                >
                  <strong>📡 About the Watchdog</strong> — We monitor the web
                  using AI-powered searches for each item you add. When new
                  articles are found, they appear here. Click{" "}
                  <strong>"Generate Post"</strong> on any update to turn it into
                  a LinkedIn post with one click.
                </div>
              )}
            </>
          )}
          ...
          <div
            ...
            onClick={() => {
              if (!creationModal.loading)
                setCreationModal((p) => ({ ...p, visible: false }));
            }}
          >
            <div
              style={{
            ...
            >
              ...
              >
                <div
                  style={{ fontWeight: 800, fontSize: 15, color: "#111827" }}
                >
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "none",
                    fontSize: 20,
                    cursor: creationModal.loading ? "not-allowed" : "pointer",
                    color: "#6b7280",
                    padding: "4px 8px",
                    borderRadius: 6,
                    opacity: creationModal.loading ? 0.4 : 1,
                  }}
                >
                  ✕
                </button>
              ...
              <div style={{ padding: 16, maxHeight: "60vh", overflow: "auto" }}>
                <div style={{ marginBottom: 14 }}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setCreationModal((p) => ({ ...p, topic: e.target.value }))
                    }
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    fontSize: 14,
                    fontWeight: 600,
                    opacity: creationModal.loading ? 0.5 : 1,
                  }}
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={
                    creationModal.loading || !creationModal.topic.trim()
                  }
                  style={{
                    padding: "10px 24px",
                    border: "none",
