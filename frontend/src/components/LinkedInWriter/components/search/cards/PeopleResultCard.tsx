    ...
    <article style={cardStyle}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {position && (position.role || position.company) && (
          <p style={{ margin: "4px 0 0", fontSize: 12, color: "#64748b" }}>
            Current:{" "}
            {[position.role, position.company].filter(Boolean).join(" at ")}
          </p>
        )}
        <div
          style={{
        ...
        >
          {item.connections_count != null && (
            <span>{formatCount(item.connections_count)}+ connections</span>
          )}
          {item.followers_count != null && (
            <span>{formatCount(item.followers_count)} followers</span>
          )}
