...
export const LastUpdatedBanner: React.FC<{
  ...
  return (
    ...
    >
      ...
      <span>
        Analytics were last fetched from LinkedIn <strong>{relative}</strong> (
        {absolute}). Refresh to pull current metrics and compare against your
        previous snapshot.
      </span>
...
export const ComparisonPeriodBlock: React.FC<{ from: string; to: string }> = ({
  from,
  to,
}) => {
  const period = useMemo(
    () => formatLocalizedComparisonPeriod(from, to),
    [from, to],
  );
  ...
  return (
    ...
    >
      ...
      <div
        style={{ fontSize: 11, color: colors.textTertiary, marginBottom: 8 }}
      >
