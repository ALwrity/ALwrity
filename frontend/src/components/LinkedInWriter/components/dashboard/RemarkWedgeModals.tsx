...
const MetricPill: React.FC<{ icon: string; value: number; label: string }> = ({
  icon,
  value,
  label,
}) => (
...
export const RepurposeLabModal: React.FC<RepurposeLabModalProps> = ({
  ...
  return (
    ...
    >
      ...
      {loading && (
        <LoadingRow message="Fetching your top posts from LinkedIn…" />
      )}
      ...
      {!loading &&
        topPosts.map((post, idx) => (
          <TopPerformerCard
            ...
            rank={idx + 1}
            onRepurpose={(type) => {
              openInCreate(type, post.title ?? "My Post", post.text);
              onClose();
            }}
          />
        ))}
...
const TopPerformerCard: React.FC<{
  post: LinkedInPost;
  rank: number;
  onRepurpose: (type: string) => void;
}> = ({ post, rank, onRepurpose }) => {
  const rankColors = [
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {REPURPOSE_ACTIONS.map((a) => (
          <button
            key={a.type}
            type="button"
            onClick={() => onRepurpose(a.type)}
            style={{
              padding: "5px 12px",
              background: a.type === "post" ? "none" : a.accent,
              color: a.type === "post" ? a.accent : "#fff",
