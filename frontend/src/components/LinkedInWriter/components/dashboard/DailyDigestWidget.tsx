...
function buildDigest(data: ConsolidatedGrowthResponse): DigestRow[] {
  ...
  const todayPost =
    data.weekly_strategy?.daily_posts?.find((dp) =>
      dp.day?.toLowerCase().startsWith(today.toLowerCase().slice(0, 3)),
...
export const DailyDigestWidget: React.FC<DailyDigestWidgetProps> = ({
  compact = false,
}) => {
