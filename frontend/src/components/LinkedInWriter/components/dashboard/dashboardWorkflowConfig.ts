...
export const DASHBOARD_WORKFLOW_ICONS: Record<
  DashboardWorkflowIconKey,
  SvgIconComponent
> = {
  plan: EventNoteIcon,
...
export function resolveDashboardWorkflowIcon(
...
export function wedgeAnglesForIndex(
  index: number,
): Pick<DashboardWorkflowCard, "startAngle" | "endAngle"> {
  const center =
...
export const DASHBOARD_WORKFLOW_CARDS: DashboardWorkflowCard[] = CARD_DEFS.map(
  (card, index) => ({
    ...card,
...
export const MOBILE_PRIMARY_WORKFLOW_IDS: readonly DashboardWorkflowCardId[] = [
  "plan",
  "create",
];
