...
export const TodayGrowthWalkthrough: React.FC<TodayGrowthWalkthroughProps> = ({
  variant = "main",
}) => {
  const { userId } = useAuth();
  const currentWorkflow = useWorkflowStore((s) => s.currentWorkflow);
  const loadTodayWorkflow = useWorkflowStore((s) => s.loadTodayWorkflow);
  const generateDailyWorkflow = useWorkflowStore(
  ...
  const allTasksDone = pillarTasks.every(
    (t) =>
      t.status === "completed" ||
      t.status === "skipped" ||
      optimisticDone.has(t.id),
  );
  ...
  const handleTaskCheck = useCallback(
    async (task: TodayTask) => {
      if (
        optimisticRef.current.has(task.id) ||
  ...
  const isNotFound =
    ...
    (errMsg?.toLowerCase().includes("not found") ||
      errMsg?.toLowerCase().includes("no workflow") ||
      errMsg?.includes("404"));
  ...
  const hasAnyTasks =
    currentWorkflow?.tasks && currentWorkflow.tasks.length > 0;
  ...
  const totalEstMinutes = sortedTasks.reduce(
  ...
  const renderPillarSteps = () => (
    <div
      className="linkedin-growth-pillar-steps"
      role="tablist"
      aria-label="Growth workflow pillars"
    >
  ...
  const renderPanelContent = () => {
          ...
          <div className="linkedin-growth-dropdown-loading">
            ...
            {generationProgress ? (
              ...
              <div style={{ fontSize: 12, color: "#999" }}>
                {isToday
                  ? "Preparing your LinkedIn growth workflow"
                  : `Loading tasks for ${formatDateLabel(selectedDate)}`}
              </div>
            )}
          </div>
        </div>
    ...
    if (phase === "error") {
      const noTasksPast =
        !isToday && !storeError && errMsg?.includes("No tasks recorded");
      return (
        <>
          <div className="linkedin-growth-dropdown-body">
            <div className="linkedin-growth-dropdown-empty">
              <div style={{ fontSize: 28, marginBottom: 10 }}>
                {isNotFound || noTasksPast ? "📋" : "⚠️"}
              </div>
              <div
                style={{
                  fontSize: 14,
                  fontWeight: 700,
                  color: isNotFound || noTasksPast ? "#1a1a2e" : "#d32f2f",
                  marginBottom: 6,
                }}
              >
                {isNotFound
              ...
              <div style={{ fontSize: 12, color: "#666", lineHeight: 1.5 }}>
                {isNotFound
                  ...
                  : noTasksPast
                    ...
                    : errMsg ||
                      "Could not load growth tasks. Please try again."}
              </div>
            </div>
          </div>
          ...
          <div className="linkedin-growth-dropdown-body">
            <div className="linkedin-growth-dropdown-empty">
              ...
              <div style={{ fontSize: 13, color: "#666", lineHeight: 1.5 }}>
                {hasAnyTasks
                  ? isToday
                    ? "You've completed today's growth tasks across all pillars."
                    : `All tasks completed for ${formatDateLabel(selectedDate)}.`
                  : isToday
                    ? "No growth tasks for today. Check back tomorrow!"
                    : `No tasks for ${formatDateLabel(selectedDate)}.`}
              </div>
            </div>
          </div>
          <div className="linkedin-growth-dropdown-footer">
            <button type="button" onClick={closeDropdown} style={btnPrimary}>
              Finish
            </button>
          </div>
        </>
              ...
              {sortedTasks.map((task) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      style={{ fontSize: 10, color: "#bbb", flexShrink: 0 }}
                    >
    ...
    const doneCount = pillarTasks.filter(
      (t) =>
        t.status === "completed" ||
        t.status === "skipped" ||
        optimisticDone.has(t.id),
                  ...
                  >
                    <input
                      ...
                      style={{
                        ...
                        width: 16,
                        height: 16,
                        accentColor: pillarMeta?.color || "#6366f1",
                      }}
                    />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div
        ...
        <div className="linkedin-growth-dropdown-footer">
          <div style={progressBarContainer}>
            <div
              style={{
                ...progressBarFill,
                width: `${(doneCount / Math.max(pillarTasks.length, 1)) * 100}%`,
                background: pillarMeta?.color || "#6366f1",
              }}
            />
          </div>
  ...
  const panelTitle =
    phase === "active"
      ...
      : phase === "briefing"
        ? isToday
          ? "Today's Growth Briefing"
          : `Growth Briefing — ${formatDateLabel(selectedDate)}`
        : phase === "completed"
          ? "Growth Complete"
          : phase === "error"
            ? "Growth Tasks"
  ...
  return (
    ...
    >
      ...
      {open && !isMobileTab && (
        <div
          className="linkedin-growth-dropdown-panel"
          role="dialog"
          aria-label="Today's Growth Tasks"
        >
          <div className="linkedin-growth-dropdown-header">
            ...
            <button
              type="button"
              className="linkedin-growth-dropdown-close"
              onClick={closeDropdown}
              aria-label="Close growth tasks"
            >
              ×
            </button>
          </div>
