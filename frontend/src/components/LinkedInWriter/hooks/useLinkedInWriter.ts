...
export function useLinkedInWriter() {
  ...
  const [draft, setDraft] = useState(() => {
    try {
      return sessionStorage.getItem("li_draft") || "";
    } catch {
      return "";
    }
  });
  ...
  const [outlineSections, setOutlineSections] = useState<
    LinkedInOutlineSection[]
  >([]);
  const [outlineTitleSuggestions, setOutlineTitleSuggestions] = useState<
    string[]
  >([]);
  ...
  const [userPreferences, setUserPreferences] =
    useState<LinkedInPreferences>(getPreferences());
  ...
  const trackActionUsage = useCallback((actionName: string) => {
    ...
    const updatedActions = [
      ...(currentPrefs.last_used_actions || []),
      actionName,
    ].slice(-5);
    savePreferences({ last_used_actions: updatedActions });
    setUserPreferences((prev) => ({
      ...prev,
      last_used_actions: updatedActions,
    }));
    ...
    window.dispatchEvent(
      new CustomEvent("linkedinwriter:loadingStart", {
        detail: {
          ...
          message: "Researching and writing your LinkedIn post...",
        },
      }),
    );
    window.dispatchEvent(
    ...
    try {
      const res = await linkedInWriterApi.generatePost({
        ...
        include_call_to_action:
          params?.include_call_to_action ??
          prefs.include_call_to_action ??
          true,
        research_enabled:
          params?.research_enabled ?? prefs.research_enabled ?? true,
        search_engine: mapSearchEngine(
          params?.search_engine || prefs.search_engine,
        ),
        max_length:
          params?.max_length ||
          prefs.max_length ||
        ...
        window.dispatchEvent(
          new CustomEvent("linkedinwriter:progressStep", {
            detail: {
              id: "finalize",
              status: "completed",
              message: "Post optimized for LinkedIn engagement",
            },
          }),
        );
        window.dispatchEvent(
      ...
      window.dispatchEvent(
        new CustomEvent("linkedinwriter:progressError", {
          detail: { id: "finalize", details: error.message },
        }),
      );
    ...
    async (params?: any) => {
      ...
      window.dispatchEvent(
        new CustomEvent("linkedinwriter:loadingStart", {
          detail: {
            ...
            message: "Researching and writing your LinkedIn article...",
          },
        }),
      );
      window.dispatchEvent(
      ...
      try {
        const sectionOverride =
          outlineSections.length > 0 ? outlineSections : undefined;
        const res = await linkedInWriterApi.generateArticle({
          ...
          include_images:
            params?.include_images ?? prefs.include_images ?? true,
          seo_optimization:
            params?.seo_optimization ?? prefs.seo_optimization ?? true,
          research_enabled:
            params?.research_enabled ?? prefs.research_enabled ?? true,
          search_engine: mapSearchEngine(
            params?.search_engine || prefs.search_engine,
          ...
          window.dispatchEvent(
            new CustomEvent("linkedinwriter:progressStep", {
              detail: {
                id: "finalize",
                status: "completed",
                message: "Article formatted for LinkedIn publishing",
              },
            }),
          );
          window.dispatchEvent(
  ...
  const generateCarousel = useCallback(async (params?: any) => {
    ...
    window.dispatchEvent(
      new CustomEvent("linkedinwriter:loadingStart", {
        detail: {
          action: "generateLinkedInCarousel",
          message: "Researching and building your LinkedIn carousel...",
        },
      }),
    );
    window.dispatchEvent(
    ...
    try {
      const res = await linkedInWriterApi.generateCarousel({
        ...
        include_cover_slide:
          params?.include_cover_slide ?? prefs.include_cover_slide ?? true,
        include_cta_slide:
          params?.include_cta_slide ?? prefs.include_cta_slide ?? true,
        ...
        window.dispatchEvent(
          new CustomEvent("linkedinwriter:progressStep", {
            detail: {
              id: "finalize",
              status: "completed",
              message: "Carousel optimized for mobile viewing",
            },
          }),
        );
        window.dispatchEvent(
  ...
  const generateVideoScript = useCallback(async (params?: any) => {
    ...
    window.dispatchEvent(
      new CustomEvent("linkedinwriter:loadingStart", {
        detail: {
          action: "generateLinkedInVideoScript",
          message: "Researching and writing your video script...",
        },
      }),
    );
    window.dispatchEvent(
    ...
    try {
      const res = await linkedInWriterApi.generateVideoScript({
        ...
        include_captions:
          params?.include_captions ?? prefs.include_captions ?? true,
      });
      clearInterval(progressInterval);
      if (res.success && res.data) {
        while (stepIndex < progressStepIds.length) {
        ...
        window.dispatchEvent(
          new CustomEvent("linkedinwriter:progressStep", {
            detail: {
              id: "finalize",
              status: "completed",
              message: "Script ready for production",
            },
          }),
        );
        window.dispatchEvent(
  ...
  useEffect(() => {
    const loadInitialData = () => {
      try {
        ...
        console.warn(
          "[LinkedIn Writer] Failed to initialize from localStorage:",
          error,
        );
      }
    };
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "linkedinwriter:progressInit",
      handleProgressInit as EventListener,
    );
    window.addEventListener(
      "linkedinwriter:progressStep",
      handleProgressStep as EventListener,
    );
    window.addEventListener(
      "linkedinwriter:progressComplete",
      handleProgressComplete as EventListener,
    );
    window.addEventListener(
      "linkedinwriter:progressError",
      handleProgressError as EventListener,
    );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "linkedinwriter:updateGroundingData",
      handleGroundingDataUpdate as EventListener,
    );
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      ...
      handleUpdateDraft as EventListener,
    );
    window.addEventListener(
      "linkedinwriter:appendDraft",
      handleAppendDraft as EventListener,
    );
    window.addEventListener(
      "linkedinwriter:assistantMessage",
      handleAssistantMessage as EventListener,
    );
    window.addEventListener(
      "linkedinwriter:applyEdit",
      handleApplyEdit as EventListener,
    );
    window.addEventListener(
      "linkedinwriter:loadingStart",
      handleLoadingStart as EventListener,
    );
    window.addEventListener(
      "linkedinwriter:loadingEnd",
  ...
  const generateOutline = useCallback(async (params?: any) => {
    ...
    window.dispatchEvent(
      new CustomEvent("linkedinwriter:loadingStart", {
        detail: {
          action: "generateLinkedInOutline",
          message: "Planning article sections...",
        },
      }),
    );
    ...
    try {
      ...
      return {
        success: false,
        error: res.error || "Outline generation failed",
      };
    } catch (error: any) {
      window.dispatchEvent(new CustomEvent("linkedinwriter:loadingEnd"));
      return {
