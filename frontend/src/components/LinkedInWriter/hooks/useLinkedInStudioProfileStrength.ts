...
export function useLinkedInStudioProfileStrength() {
    ...
    getLinkedInProfileFoundation()
      .then((data) => {
        ...
        if (validation) {
          dispatchProfileStrengthUpdated(
            validation,
            data.ai_profile_intelligence ?? null,
          );
        }
      })
      .catch(() => {
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      const detail = (event as CustomEvent<ProfileStrengthUpdatedDetail>)
        .detail;
      if (detail?.profileValidation) {
        setProfileValidation(detail.profileValidation);
        setProfileStrengthPercent(
          getDisplayProfileStrengthPercent(detail.profileValidation),
        );
      }
    };
  ...
  return {
    ...
    strengthLabel: getProfileStrengthDisplayLabel(
      profileValidation,
      profileStrengthPercent,
    ),
