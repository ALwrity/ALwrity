...
const RegisterLinkedInActions: React.FC = () => {
    ...
    parameters: [
      {
        ...
        required: true,
        description:
          "Type of LinkedIn content (post, article, carousel, video_script)",
      },
      {
        name: "topic",
        type: "string",
    ...
    handler: async (args: any) => {
      try {
        const response = await apiClient.post(
          "/api/linkedin/generate-image-prompts",
          {
  ...
  useCopilotActionTyped({
    ...
    parameters: [
      ...
      },
      {
        ...
        required: true,
        description:
          "Content context including topic, industry, content_type, and style",
        attributes: [
          {
            name: "topic",
            type: "string",
          ...
          },
          {
            ...
            type: "string",
            required: true,
            description: "Type of content (post, article, carousel)",
          },
          {
            name: "style",
            type: "string",
  ...
  useCopilotActionTyped({
    ...
    description:
      "Edit an existing LinkedIn image using natural language (change style, background, colors, etc.). Requires an image_id from a previously generated LinkedIn image.",
    parameters: [
      {
        name: "image_id",
        type: "string",
        required: true,
        description: "ID of the previously generated LinkedIn image to edit",
      },
      {
        name: "prompt",
        type: "string",
        required: true,
        description:
          'Natural language description of desired edits (e.g., "Make the background blue", "Add more professional look")',
      },
      {
        name: "content_context",
        type: "object",
        required: true,
        description: "Content context including topic, industry, content_type",
        attributes: [
          {
            name: "topic",
            type: "string",
  ...
  useCopilotActionTyped({
    ...
    description:
      "Generate a professional LinkedIn post with industry insights and engagement optimization",
    parameters: [
      ...
      { name: "tone", type: "string", required: false },
      {
        ...
        required: false,
        description:
          "Whether to refine existing content instead of creating new",
      },
    ],
    handler: async (args: any) => {
      ...
      const res = await linkedInWriterApi.generatePost({
        ...
        include_hashtags:
          args?.include_hashtags ?? prefs.include_hashtags ?? true,
        include_call_to_action:
          args?.include_call_to_action ?? prefs.include_call_to_action ?? true,
        research_enabled:
          args?.research_enabled ?? prefs.research_enabled ?? true,
        search_engine: mapSearchEngine(
          args?.search_engine || prefs.search_engine,
        ),
        max_length:
          args?.max_length ||
          prefs.max_length ||
      ...
      if (res.success && res.data) {
        ...
        const fullContent = assembleLinkedInPostContent({
          ...
          includeHashtags:
            args?.include_hashtags ?? prefs.include_hashtags ?? true,
          includeCallToAction:
            args?.include_call_to_action ??
            prefs.include_call_to_action ??
            true,
        });
        ...
        console.log(
          "[LinkedIn Writer] Research sources:",
          res.research_sources,
        );
        console.log("[LinkedIn Writer] Citations:", res.data?.citations);
        console.log(
          "[LinkedIn Writer] Quality metrics:",
          res.data?.quality_metrics,
        );
        console.log(
          "[LinkedIn Writer] Grounding enabled:",
          res.data?.grounding_enabled,
        );
        ...
        console.log(
          "[LinkedIn Writer] Sending draft update:",
          fullContent?.substring(0, 100) + "...",
        );
        console.log(
          "[LinkedIn Writer] Full content length:",
          fullContent?.length,
        );
        ...
        const recommendations =
          res.data?.quality_metrics?.recommendations || [];
        if (recommendations.length > 0) {
          // Create a markdown-formatted message with recommendations
          const recommendationsMarkdown = recommendations
      ...
      window.dispatchEvent(new CustomEvent("linkedinwriter:loadingEnd"));
      window.dispatchEvent(
        new CustomEvent("linkedinwriter:progressError", {
          detail: { id: "finalize", details: res.error },
        }),
      );
      return {
        success: false,
        message: res.error || "Failed to generate LinkedIn post",
      };
    },
  });
  ...
  useCopilotActionTyped({
    ...
    description:
      "Generate a comprehensive LinkedIn article with thought leadership content",
    parameters: [
      { name: "topic", type: "string", required: false },
      { name: "industry", type: "string", required: false },
      { name: "tone", type: "string", required: false },
    ...
    handler: async (args: any) => {
      ...
      const res = await linkedInWriterApi.generateArticle({
        ...
        seo_optimization:
          args?.seo_optimization ?? prefs.seo_optimization ?? true,
        research_enabled:
          args?.research_enabled ?? prefs.research_enabled ?? true,
        search_engine: mapSearchEngine(
          args?.search_engine || prefs.search_engine,
      ...
      return {
        success: false,
        message: res.error || "Failed to generate LinkedIn article",
      };
    },
  });
  ...
  useCopilotActionTyped({
    ...
    description:
      "Generate a LinkedIn carousel with multiple slides for visual content",
    parameters: [
      { name: "topic", type: "string", required: false },
      { name: "industry", type: "string", required: false },
    ...
    handler: async (args: any) => {
      ...
      return {
        success: false,
        message: res.error || "Failed to generate LinkedIn carousel",
      };
    },
  });
  ...
  useCopilotActionTyped({
    ...
    description:
      "Generate a LinkedIn video script with hook, content, and captions",
    parameters: [
      { name: "topic", type: "string", required: false },
      { name: "industry", type: "string", required: false },
    ...
    handler: async (args: any) => {
      ...
      const res = await linkedInWriterApi.generateVideoScript({
        ...
        include_captions:
          args?.include_captions ?? prefs.include_captions ?? true,
      });
      ...
      return {
        success: false,
        message: res.error || "Failed to generate LinkedIn video script",
      };
    },
  });
  ...
  useCopilotActionTyped({
    name: "improveContent",
    description:
      "Improve specific aspects of LinkedIn content based on AI recommendations",
    parameters: [
    ...
    handler: async (args: any) => {
      ...
      if (
        recommendation.toLowerCase().includes("factual accuracy") ||
        recommendation.toLowerCase().includes("accuracy")
      ) {
        ...
        recommendation.toLowerCase().includes("professional tone") ||
        recommendation.toLowerCase().includes("tone")
      ) {
        ...
        recommendation.toLowerCase().includes("citation") ||
        recommendation.toLowerCase().includes("sources")
      ) {
        ...
        recommendation.toLowerCase().includes("industry relevance") ||
        recommendation.toLowerCase().includes("relevance")
      ) {
      ...
      const actionItemsMarkdown = actionItems
  ...
  useCopilotActionTyped({
    ...
    description:
      "Help users improve their LinkedIn content based on natural language requests",
    parameters: [{ name: "user_request", type: "string", required: true }],
    handler: async (args: any) => {
      ...
      if (
        request.includes("help me improve") ||
        request.includes("how to improve") ||
        ...
        else if (request.includes("accuracy") || request.includes("factual"))
          aspect = "factual accuracy";
        else if (request.includes("citation") || request.includes("source"))
          aspect = "citation coverage";
        else if (request.includes("relevance") || request.includes("industry"))
          aspect = "industry relevance";
        else if (request.includes("grammar") || request.includes("language"))
      ...
      if (
        request.includes("recommendation") ||
        request.includes("suggestion")
      ) {
        return {
          success: true,
  ...
  useCopilotActionTyped({
    ...
    handler: async (args: any) => {
      ...
      return {
        success: false,
        message: res.error || "Failed to generate LinkedIn comment response",
      };
    },
  });
  ...
  useCopilotActionTyped({
    ...
    description:
      "Optimize LinkedIn profile sections for better professional visibility",
    parameters: [
    ...
    handler: async () => {
      showToastNotification(
        "LinkedIn Profile Optimization is coming soon! Stay tuned for this feature.",
        "info",
      );
  ...
  useCopilotActionTyped({
    ...
    description:
      "Generate an engaging LinkedIn poll with professional questions",
    parameters: [
      { name: "topic", type: "string", required: false },
      { name: "industry", type: "string", required: false },
    ...
    handler: async () => {
      showToastNotification(
        "LinkedIn Poll Generation is coming soon! Stay tuned for this feature.",
        "info",
      );
      return { success: false, message: "Feature coming soon" };
    },
  ...
  useCopilotActionTyped({
    ...
    handler: async () => {
      showToastNotification(
        "LinkedIn Company Update Generation is coming soon! Stay tuned for this feature.",
        "info",
      );
      return { success: false, message: "Feature coming soon" };
    },
