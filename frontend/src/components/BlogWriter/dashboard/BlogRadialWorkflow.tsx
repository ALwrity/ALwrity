...
import type { BlogRadialLayout } from "./blogRadialLayout";
import { WedgeMetricBadge, type BlogWedgeMetrics } from "./WedgeMetricBadge";

interface BlogRadialWorkflowProps {
  layout: BlogRadialLayout;
...
export const BlogRadialWorkflow: React.FC<BlogRadialWorkflowProps> = ({
  layout,
  onCardAction,
  connected = true,
  metrics = {},
  ...
  const handleCardAction = (cardId: BlogWorkflowCardId) => {
  ...
  const orderedCards = [...BLOG_WORKFLOW_CARDS].sort((a, b) => {
  ...
  const renderWedge = (card: (typeof BLOG_WORKFLOW_CARDS)[number]) => {
    ...
    return (
      <g
        ...
        className={[
          "blog-workflow-wedge",
          isConnectLocked && "blog-workflow-wedge--connect-locked",
        ]
          .filter(Boolean)
          .join(" ")}
        data-tour={`blog-wedge-${card.id}`}
      ...
      >
        {isConnectLocked && (
          <title>Connect WordPress or Wix to unlock {card.title}</title>
        )}
        <path
          className="blog-workflow-wedge-base"
          d={wedgePath}
          fill={
            isConnectLocked
        ...
        >
          ...
          >
            ...
            {metricLabel && <WedgeMetricBadge label={metricLabel} accent={card.accent} />}
            {isRecommended && (
              <div
                style={{
                  marginTop: 4,
  ...
  return (
    <svg
      ...
      aria-label="Blog Writer workflow"
    >
      <defs>
        <radialGradient id="blog-wedge-ring-glow" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor="rgba(25,118,210,0.05)" />
          <stop offset="35%" stopColor="rgba(25,118,210,0.2)" />
          <stop offset="65%" stopColor="rgba(156,39,176,0.1)" />
          <stop offset="100%" stopColor="rgba(25,118,210,0)" />
        </radialGradient>
        <linearGradient id="blogWedgeFill" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor="#ffffff" />
          <stop offset="100%" stopColor="#eaf2fa" />
        </linearGradient>
        <filter id="blog-atmospheric-glow" x="-60%" y="-60%" width="220%" height="220%">
      ...
      <circle
        ...
        cy={centerY}
        r={glowR}
        fill="url(#blog-wedge-ring-glow)"
        filter="url(#blog-atmospheric-glow)"
        style={{ pointerEvents: "none" }}
      />
      <style>{`
        .blog-workflow-wedge:focus-visible .blog-workflow-wedge-base {
          stroke-width: 3px;
        }
      `}</style>
