...
# Phase 3.6: surface the silent "salt missing → derived secret is empty bytes"
# failure by exposing ``validate_file_encryption_salt``. Tests stand up the
# module under the alias ``_phase36_afc`` and look up this function at module
# scope, so it MUST live at module level (not inside the class).
